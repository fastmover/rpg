...
#			print(event.position)
			print('-- start --')
			print('event.position global:')
			print(event)
			print(_is_point_inside_joystick_area(event.position))
			if _is_point_inside_joystick_area(event.position) and _touch_index == -1:
				print('passed _is_point_inside_joystick_area()')
...
func _is_point_inside_joystick_area(point: Vector2) -> bool:
	...
	print(get_global_transform_with_canvas().get_scale().x)
